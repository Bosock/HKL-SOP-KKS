...
|---|---|---|
| 2026-08-18 | **Anleitungs-Fotos raus aus dem geteilten Zustand (Ursache des „lokal"-Pills beim Bearbeiten):** Schritt-Fotos einer Anleitung lagen als base64 im geteilten Schlüssel `hkl_guides` — als letzte Stelle der App überhaupt. Folge: Bei JEDER Änderung an der Anleitung (auch beim Korrigieren eines Wortes) wanderte der gesamte Bildbestand zum Server, und der Gerätespeicher war nach ~6–10 Fotos voll; sichtbar wurde beides nur als oranges „lokal"-Pill, also als „nichts kommt mehr an". Fotos nehmen jetzt denselben Weg wie Produkt- und Bestellfotos: verkleinert, **einzeln auf den Server** (`/api/media`), am Schritt steht nur die Adresse (~40 statt ~250.000 Zeichen); Rückfall auf verkleinertes base64, falls der Medienspeicher fehlt. **Altbestand zieht nach** (`medMigriereAltbestand` deckt jetzt auch `GUIDES` ab, schonend im Hintergrund, idempotent). Dazu **Statusanzeige entschärft**: „zu groß" und „kein Netz" tragen dasselbe Pill, gehen aber verschieden aus — das 413-Pill heißt jetzt „zu groß" statt „lokal"; Selbsttext zum Speicherplatz sagt nicht länger, Fotos zählten nicht mit. Cache v72; +10 Unit-Tests (`test/anleitungsfotos.test.js`, `test/medien.test.js`), neue E2E-Suite `anleitungsfotos.js` (14 Checks, beweist im Browser: 39 KB → 0 KB im Zustand, Sync grün); 1014 Unit + `npm run check` grün. | Nutzer-Rückmeldung (Screenshot: „lokal" im Anleitungs-Editor) |
| 2026-08-11 | **5-MB-Grenze beseitigt (Produkt- & Bestellfotos raus aus dem localStorage):** Diese Fotos lagen als base64 IM geteilten Zustand (`GTINDB`, `BEST`) und damit im localStorage, den der Browser bei ~5 MB deckelt — danach ging jede weitere Änderung still verloren. Sie nehmen jetzt denselben Weg wie die Eintrags-Bilder: verkleinert, **einzeln auf den Server** (`/api/media`, Inhalts-Fingerabdruck), im Zustand nur die 32-stellige Kennung. **Client-Kennung == Server-Kennung** (SHA-256, per Test bewiesen) → dieselben Bytes belegen nie zweimal Platz. **Offline-fest:** lokaler Spiegel in IndexedDB + globaler `<img>`-Fehler-Fänger zeigen ein frisch aufgenommenes Foto sofort, auch ohne Empfang. **Einmalige Migration** des Altbestands läuft schonend im Hintergrund und befreit den localStorage. `hkl_bestellungen` zusätzlich ins Backup aufgenommen. Cache v69; +5 Unit-Tests (`test/medien.test.js`), 1004 Unit + 41 E2E grün. | Betreiber: „das 5 MB Limit muss beseitigt werden!" |

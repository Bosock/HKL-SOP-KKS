...
test('install precaches the app shell', async () => {
  ...
  await env.install();
  const shell = env.stores.get('hkl-shell-v40');
...
test('activate drops stale hkl caches, keeps current ones', async () => {
  ...
  env.box.fetchImpl = (input) => mkRes('shell:' + input);
  await env.install();
  await env.caches.open('hkl-shell-v39'); // simulate an old generation
  await env.activate();
  const names = await env.caches.keys();
  assert.ok(!names.includes('hkl-shell-v39'), 'old cache removed');
  assert.ok(names.includes('hkl-shell-v40'), 'current shell cache kept');
});
